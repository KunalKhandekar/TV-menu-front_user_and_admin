...
export default function TVList({ restaurantId, get_tv }) {
  const { getTVs, addTV, updateTV, deleteTV } = useContext(RestaurantContext);
  const [tvs, setTVs] = useState(get_tv);
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <ScrollArea className="h-[300px]">
          {tvs.length === 0 ? (
            <div className="text-center py-4 text-muted-foreground">No TVs added yet.</div>
          ) : (
            tvs.map((tv) => (
              <div key={tv._id} className="flex items-center justify-between p-2 hover:bg-accent hover:text-accent-foreground rounded-md">
                <span>{tv.name}</span>
                <div className="space-x-2">
                  <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <form onSubmit={handleUpdateTV}>
                        <div className="grid gap-4 py-4">
                          <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="edit-tv-name" className="text-right">
                              Name
                            </Label>
                            <Input
                              id="edit-tv-name"
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          ...
                          TV and all associated images.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => handleDeleteTV(tv._id)} disabled={isDeleting}>
                          {isDeleting ? (
                            <>
                              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                              Deleting...
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="max-w-3xl">
                      <DialogHeader>
                        <DialogTitle>Manage Images for {tv.name}</DialogTitle>
                      </DialogHeader>
                      <ImageList restaurantId={restaurantId} tvId={tv._id} Images_List={tv?.menuImages} />
                    </DialogContent>
                  </Dialog>
                </div>
              </div>
